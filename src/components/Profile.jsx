...
import { CalendarDays, Heart, Eye, Phone, Mail } from 'lucide-react';
...
const Profile = () => {
  ...
  const [activeTab, setActiveTab] = useState('postlar'); // Default tab
  ...
  const fetchUserContent = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}user/posts/user/`, {
        method: 'GET',
      ...
      if (data.status === 'success' && data.data) {
        ...
        // setCompactUserInfo(prev => ({ ...prev, user: data.data.user }));
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log(compactUserInfo);
  }, []);

  // Memoized filters for different content types
  // Postlar: faqat rasmli postlar (media_type === 'image'), kamida 1 ta rasm, va video yo'q
  // Postlar: faqat rasmli postlar (media_type === 'image'), kamida 1 ta rasm, va video yo'q
  // Videolar: faqat video postlar (media_type === 'video'), kamida 1 ta video, va rasm yo'q
  const posts = useMemo(() => 
    content.filter((item => 
      item.type === 'post' &&
      item.media.length >= 1 &&
      ...
      item.type === 'post' &&
      item.media.length >= 1 &&
      item.media.every(m => m.media_type === 'video')
    )),
    [content]
  );

 
  // Videolar: faqat video postlar (media_type === 'video'), kamida 1 ta video


  const videos = useMemo(() => 
    content.filter(item => item.type === 'post' && item.media.length === 1 && item.media[0].media_type === 'video'),
    [content]
  );
  ...
  const renderContent = () => {
    if (loading) {
      if (activeTab === 'maqolalar') {
        return <div className="article-list">{Array.from({ length: 3 }).map((_, i) => <SkeletonCard key={i} />)}</div>;
      }
      return <SkeletonGrid />;
    }
          ...
          <div className="media-grid">
            {videos.map((post) => (
              <div key={post.id} className="media-item video-item">
                <video src={`${API_URL2}${post.media[0].file_url}`} />
                <div className="play-icon-overlay"><svg viewBox="0 0 24 24"><path fill="white" d="M8,5.14V19.14L19,12.14L8,5.14Z" /></svg></div>
              </div>
            ))}
          </div>
              ...
              <div key={article.id} className="article-card" >
                {article.media && article.media.length > 0 && (
                  <div className="article-card-image-wrapper">
                    <img src={getFullUrl(article.media[0].file_url)} alt={article.title} className="article-card-image" />
                  </div>
                )}
                <div className="article-card-content">
    ...
    <div className="profile-container">
      ...
      {compactUserInfo?.profession?.length > 0 && (
        <div style={{display:'flex',justifyContent:'left',marginTop:'-1px', marginLeft:'140px'}}>
          <span style={{
            background:'#1746a2', color:'#fff', borderRadius:20, padding:'6px 12px', fontWeight:600, fontSize:16, display:'inline-flex',alignItems:'left',gap:6
          }}>
            sulton
          </span>
        </div>
      ...
      {/* PROFILE IMAGE + STATS (Instagram style row) */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
      ...
      {compactUserInfo && (
        <div style={{textAlign:'left', padding: '0 8px', marginBottom:16}}>
          <div style={{fontWeight:700, fontSize:20, marginBottom:2}}>
            {compactUserInfo?.user?.full_name}
          </div>
          ...
          {compactUserInfo?.user?.bio && (
              <p style={{fontSize:15, color:'#333', marginBottom:8, marginTop: 0, lineHeight: 1.5}}>
                  {compactUserInfo.user.bio}
              </p>
          )}
          ...
          {compactUserInfo?.profession?.length > 0 && (
            <div style={{display:'flex', justifyContent:'flex-start', flexWrap: 'wrap', gap:6,marginBottom:8}}>
              {compactUserInfo.profession.map((p) => (
                <span key={p.id} style={{
                  background:'#eaf4ff', color:'#1746a2', borderRadius:8, padding:'2px 12px', fontSize:14, fontWeight:500
                }}>{p.name}</span>
              ))}
            </div>
          )}
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '4px'}}>
              {compactUserInfo?.user?.phone && (
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px', fontSize:15, color:'#222'}}>
                      ...
                      <a href={`${compactUserInfo.user.phone}`} style={{color: '#222', textDecoration: 'none'}}>
                          {compactUserInfo.user.phone}
                      </a>
                  </div>
              )}
              {compactUserInfo?.user?.email && (
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px', fontSize:15, color:'#222'}}>
                      ...
                      <a href={`mailto:${compactUserInfo.user.email}`} style={{color: '#222', textDecoration: 'none'}}>
                          {compactUserInfo.user.email}
                      </a>
                  </div>
              )}
      ...
      <div className="profile-tabs">
        <button className={`tab-btn ${activeTab === 'postlar' ? 'active' : ''}`} onClick={() => setActiveTab('postlar')}>Postlar</button>
        <button className={`tab-btn ${activeTab === 'videolar' ? 'active' : ''}`} onClick={() => setActiveTab('videolar')}>Videolar</button>
        <button className={`tab-btn ${activeTab === 'maqolalar' ? 'active' : ''}`} onClick={() => setActiveTab('maqolalar')}>Maqolalar</button>
      </div>
